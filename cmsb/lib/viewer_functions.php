...
function _categoryMatchesFormatRules($category, $selectedCategory, $rootDepthVisible, $childDepthVisible, $parentVisibility) {
  ...
  if ($selectedCategory) {
    ...
    if ($isChildOfSelected) {
      if ($childDepthVisible == 'all') { $showThisCategory = true; $childDepthVisible = 999; }
...
// Show attribution footer - this function is called by ob_start()
if (!defined('IS_CMS_ADMIN') && poweredByHTML() != '') { ob_start('attributionFooter'); }  // Don't call from CMS Admin, if no footer HTML
function attributionFooter($responseContent) {
  ...
  // Check for non text/html content-type.  Default is text/html if no content-type header is sent.
  $isHTML = true;

  foreach (headers_list() as $headerLine) {
    @list($key, $value) = explode(': ', $headerLine);
    if (strtolower($key) === 'content-type') {
      $value = preg_replace("/;.*$/s", "", $value);  // remove ;charset=utf-8
      if (strtolower($value) !== 'text/html') {
        $isHTML = false;
      }
    }
  }

  // if this looks like JSON, we're not serving HTML
  if ($responseContent === 'null' || json_decode($responseContent) !== NULL) { $isHTML = false; }

  // if this looks like a record number, we're not serving HTML
  if (is_numeric($responseContent)) { $isHTML = false; }

  // if this is a blank document (such as those returned by many of our AJAX functions), we're not serving HTML
  if ($responseContent == '') { $isHTML = false; }

  // if we're sending html, check for footer and add as needed
  if ($isHTML) {
    $poweredByHTML = poweredByHTML();

    // check if powered by text already exists in page
    $isFound = (mb_strpos($responseContent, $poweredByHTML) !== false);

    if (!$isFound) {
      // if not found try to insert it before closing body tag
      $responseContent = preg_replace('|</body>|i', "$poweredByHTML\n</body>", $responseContent, 1, $count);

      // if that failed, simply append it to the end of the content
      if (!$count) { $responseContent .= $poweredByHTML; }
    }
  }
