...
function removeUploads($where) {
  global $TABLE_PREFIX, $php_errormsg;
...
function _saveUpload_moveTempUploadFile($uploadedAsFilename, $targetDir, $skipUploadSecurityCheck) {
  ...
  if ($skipUploadSecurityCheck) {
    rename_winsafe($uploadedAsFilename, $saveAsFilepath) || die("Error moving uploaded file! $php_errormsg");
  }
  else {
...
function _saveUpload_fixFilePermission($filePath){
  ...
  if (!$isReadable) {
    chmod($filePath, 0666) || die("Error changing permissions on '" .htmlspecialchars($filePath). "'! $php_errormsg");
  }
}
