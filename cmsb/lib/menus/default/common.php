...
function category_exceedMaxDepth($currentDepth, $deepestDepth, $targetParentCategoryRecord = array(), $maxDepth){
  $exceedMaxDepth = false;
  
  // return false if max depth is set to unlimited (blank)
  // OR if there is no target parent category record,
  // ... which means that we want the current record to be a top category: 'None (top level category)' option is selected
