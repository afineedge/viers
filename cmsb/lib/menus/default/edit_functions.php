      ...
      <div class="col-sm-10">
        <div class="form-inline">
          ...
          <div class="input-group $styleClassWidth">
            $beforeFieldAddonHtml
            <input type="$inputType" style="width: 100%" name="$fieldname" value="$encodedValue" $maxLengthAttr class="text-input form-control" $readOnlyAttr $autoComplete/>
            $afterFieldAddonHtml
          </div>
          $descriptionHtml
        </div>
...
function _showList($fieldSchema, $record, $fieldRowClass = null) {
  ...
  // multi pulldown
    ...
    if ($prefixText) { print "<p class=\"help-block\">$prefixText</p>\n"; }
    print "<select class=\"form-control\" name='{$fieldSchema['name']}[]' multiple='multiple' size='5'>\n";
    foreach ($listOptions as $optionArray) {
      list($value,$label) = $optionArray;
      $encodedValue = htmlencode($value);
