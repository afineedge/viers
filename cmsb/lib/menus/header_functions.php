...
function getMenuLinks() {
  ...
  foreach ($menuList as $row) {
    ...
    // show menu links
      ...
      $prefixIconAttr['class']       = $row['icon-prefix-fa']   ? 'menu-prefix-icon fa ' . $row['icon-prefix-fa'] : '';
      $prefixIconHtml                = $prefixIconAttr['class'] ? tag('i', $prefixIconAttr, '') : '';
      
      // suffix icon <i attributes
      $suffixIconAttr['aria-hidden'] = 'true';
      $suffixIconAttr['class']       = $row['icon-suffix-fa']   ? 'menu-suffix-icon fa ' . $row['icon-suffix-fa']      : ''; 
      $suffixIconHtml                = $suffixIconAttr['class'] ? tag('i', $suffixIconAttr, '') : '';
...
function _getMenuList() {
  $menus = array();
  $selectedMenu = getFirstDefinedValue(@$GLOBALS['APP']['selectedMenu'], @$_REQUEST['menu'], 'home');
  $menuOrder = 0;
...
function _getAdminMenus(&$menuOrder) {
  ...
  $adminMenus[] = array(
    ...
    'icon-prefix-fa' => 'fa-calendar-check-o',  //
  ...
  if ($menu == '_outgoing_mail' || $action == 'email' || $action == 'emailSave') { // show if email settings or outgoing mail is selected
    ...
    $adminMenus[] = array(
      ...
      'menuName'       => t('Email Templates'),
      'icon-prefix-fa' => 'fa-envelope-o',
      'icon-suffix-fa' => 'fa-external-link',
      'menuOrder'      => ++$menuOrder,
      'link'           => '?menu=_email_templates',
      'visibility'     => 'requireAdmin',
      'isSelected'     => ($menu == '_email_templates'),
