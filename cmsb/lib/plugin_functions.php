...
function loadPlugins() {
  ...
  foreach ($activePluginPaths as $pathFromPluginsDir) {
    ...
    else                         { deactivatePlugin($pathFromPluginsDir); }
  }
}
...
function getPluginList() {
  ...
  if (file_exists($disableAllPluginsFile)) { return array(); }
...
function addAction($hookName, $function, $priority = 10, $acceptedArgs = null) {
  ...
  elseif ( is_object($function) && get_class($function) === 'Closure' ) {
    ...
    die(__FUNCTION__ . ": Second parameter must by a functionName or an anonymous function!");
  }
}
...
function plugin_header($title, $buttons = '', $showDefaultButton = true) {
...
// pluginAction_addHandlerAndLink('Google', 'http://www.google.com/');
// pluginAction_addHandlerAndLink("<a href='http://www.google.com'>Google</a>");
