...
// START MODIFICATIONS ----------------------------------------------------------------------------
  ...
  // If WYWISYG isn't appearing search for "TINYMCE_COMPRESSION" and set to false
  $GLOBALS['TINYMCE_DISABLE_COMPRESSION'] = false;

// - Original source file can be found here: https://github.com/tinymce/tinymce_compressor/blob/master/tinymce.gzip.php
...
// - Changed cache prefix to 'tinymce-temp-' so it's clear it's a temp file
// - Additional CUSTOM CHANGES below have comment added: // CUSTOM CHANGES
// END MODIFICATIONS ------------------------------------------------------------------------------
...
class TinyMCE_Compressor {
    ...
    "themes"     => "",
    "languages"  => "",
    "disk_cache" => true,   // CUSTOM CHANGES: set to true to enable disk caching
    "expires"    => "30d",
    "cache_dir"  => '',     // CUSTOM CHANGES: set below in __construct
    "compress"   => true,   // CUSTOM CHANGES: set below in __construct
    "files"      => "",
    "source"     => false,  // CUSTOM CHANGES: set to false to use minified js files
	);
   ...
   */
    ...
    self::$defaultSettings['cache_dir'] = realpath(dirname(__FILE__) . "/../../data/");            // CUSTOM CHANGES
    if ($GLOBALS['TINYMCE_DISABLE_COMPRESSION']) { self::$defaultSettings['compress'] = false; }   // CUSTOM CHANGES
   ...
   */
    ...
    $tinymceDir = dirname(__FILE__);
		...
		$allFilesModifiedTimes = ''; // CUSTOM CHANGES: Hash file modified dates
		...
		for ($i = 0; $i < count($allFiles); $i++) {
			...
			if ($this->settings["source"] && file_exists($file . ".js")) {
				...
				$file .= ".min.js";
			} else if (file_exists($file . ".js")) { // CUSTOM CHANGES: If we failed to load the minified version, try the full version.
				$file .= ".js";
			} else {
				$file = "";
			}

			if ($file) { $allFilesModifiedTimes .= ',' . filemtime(__DIR__."/$file"); } // CUSTOM CHANGES: Add file modified time

			$allFiles[$i] = $file;
		}

		// Generate hash for all files
		$hash = md5(implode('', $allFiles) . $allFilesModifiedTimes . $tinymceDir); // CUSTOM CHANGES: Include file modified dates and base tinymce dir in hash (so changes to those cause the cache to be invalidated)
		...
		// Set cache file name
		$cacheFile = $this->settings["cache_dir"] . "/tinymce-temp-" . $hash . ($supportsGzip ? ".gz" : ".js"); // CUSTOM CHANGES: Changed tinymce.gzip- to tinymce-temp-
